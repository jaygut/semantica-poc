"""TNFD Disclosure tab for MARIS v3 Intelligence Platform.

Generates a robust, first-class TNFD LEAP disclosure with alignment
scoring, per-pillar breakdowns, gap analysis, and downloadable outputs.
Each section is independently error-handled so partial failures do not
collapse the entire tab.
"""
...
""",
    ...
    cols = st.columns(4)
    for i, (prefix, ps) in enumerate(pillar_scores.items()):
        color = ps["color"]
        pct = ps["pct"]
        with cols[i]:
            st.markdown(
...
def render_tnfd_compliance(
    ...
    **kwargs: Any,
    ...
    ----------
    data:
        Static bundle or case study JSON (used for fallback only; the
        disclosure generator reads from disk).
    site:
